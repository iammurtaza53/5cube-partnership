  ...
  <!---category component model start---->
    ...
    <v-dialog v-model="dialog" persistent max-width="500">
      <template v-slot:activator="{ props }">
        ...
        <v-card>
          <v-card-title>
            <span class="text-h5">{{types}} {{Edit ? "Edit":"category"}}</span>
          </v-card-title>
          ...
          <v-card-text>
            <v-container>
              <v-row>
                <v-col>
                  <v-text-field label=" Name*" placeholder="Enter Name" required v-model="cname"></v-text-field>
                </v-col>
              </v-row>
              <v-row no-gutters>
                <v-col>
                  <v-select v-model="ctype" :items="['Expense', 'Income']" label="Category type*" required></v-select>
                </v-col>
              </v-row>
            </v-container>
            <!-- </form> -->
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="white" :style="{ backgroundColor: '#e91e62' }" elevation="4" @click="dialog = false"
            >Close</v-btn>
            <v-btn color="white" :style="{ backgroundColor: 'blue' }" elevation="4"
              :disabled="disablebtn" @click="createCategories(); dialog = false;">Save</v-btn>
          </v-card-actions>
        </v-card>
    </v-dialog>
